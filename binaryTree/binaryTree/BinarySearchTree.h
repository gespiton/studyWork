...
public:
	...
	BinarySearchTree(BinarySearchTree&&rhs) :root(clone(rhs.root)) {}
    ~BinarySearchTree()
    {
        makeEmpty(root);
    }

#pragma region studyWrok
    ...
    void inOrderTravesal(BinaryNode* t, int depth)
    {
        ...
        {
            inOrderTravesal(t->left, depth +1);
            output(t->element, depth);
            inOrderTravesal(t->right, depth + 1);
        }
    }
	...
	int countAtHeight(int height)const {
		if (isEmpty())
		{
			return 0;
		}
		...
		{
			return countAtHeight(root, 0, height);
		}
	}
	int countAtHeight(const BinaryNode* t, int depth, int desDepth)const {
		if (t==nullptr)
		{
			return 0;
		}
		...
		{
			return countAtHeight(t->left, depth + 1, desDepth) + countAtHeight(t->right, depth + 1, desDepth);
		}
	}
	

	void printPathToLeave() {
		if (isEmpty())
		{
			cout << "empty tree" << endl;
		}
		vector<int>route;
		route.push_back(root->element);
		walk(root->left, route);
		walk(root->right, route);

	}
	void walk(const BinaryNode* t, vector<int> route) {
		if (t==nullptr)
		{
			for each(int i in route) {
				cout << " -> " << i;
			}
			cout << endl;
		}
		else
		{
			...
			{
				t->left == nullptr ? walk(t->right, route) : walk(t->left, route);
			}
			else
			{
				walk(t->left, route);
